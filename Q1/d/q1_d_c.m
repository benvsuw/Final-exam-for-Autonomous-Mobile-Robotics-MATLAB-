...
xR = xMax-xMin;
%tol = 1; % radius for nearest neighbours - rectilinear
% Get milestones
nS = row*col*2; % Min allowed is (M-1)*(N-1) to hit all turns
...
title(sprintf('PRM on a %dx%d maze from c)',col,row));
...
for i = 1:nM
    ...
    for j=1:p
        ...
        if (i<cur)
            ...
            if ((checkLine(1) == checkLine(2)) ||  ...  % Rectilinear 
                (checkLine(3) == checkLine(4))) % for part b)
...
% Info for part a)
disp(sprintf('Number of nodes is %d', length(samples(:,1))));
